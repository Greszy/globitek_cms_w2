<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    } elseif (!check_state($state['name'])) {
      $errors[] = "That is not a state name.";
    ...
    } elseif (!check_code($state['code'])) {
      $errors[] = "That is not a state code.";
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    }   elseif (!has_length($salesperson['email'], array('min' => 2,'max' => 255)) || !check_email($salesperson['email'])) {
      $errors[] = "Email must be less than 255 characters and contain small or capital letter, integers and _ @ . and -. "; 
    ...
    } elseif (!has_length($salesperson['phone'], array('min' => 2,'max' => 255)) || !check_phone($salesperson['phone'])) {
      $errors[] = "Phone must be less than 255 characters and follow this format: (XXX) XXX-XXXX.";
    }
  ...
  function update_salesperson($salesperson) {
    ...
    $salesperson['last_name'] = mysqli_real_escape_string($db, $salesperson['last_name']);
    $salesperson['email'] = mysqli_real_escape_string($db, $salesperson['email']);
    $salesperson['username'] = mysqli_real_escape_string($db, $salesperson['username']);
    // TODO add SQL
  ...
  function validate_user($user, $errors=array()) {
    ...
    } elseif (!has_length($user['username'], array('min' => 2,'max' => 255)) || !check_username($user['username'])) {
      $errors[] = "Username must be less than 255 characters and contain small or capital letter, integers and _.";
    }
    return $errors;
  }
  ...
  function find_user_id_by_name($user) {
      global $db;
